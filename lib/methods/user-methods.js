(function() {
  var Boom, Hoek, PageResult, UserMethods, _, bcrypt, fnUnprocessableEntity, i18n, isObjectId, mongooseRestHelper, passgen,
  ...
  module.exports = UserMethods = (function() {
    ...
    function UserMethods(models) {
      ...
      this.resetPasswordToken = bind(this.resetPasswordToken, this);
      this.resetPassword = bind(this.resetPassword, this);
      this.removeRoles = bind(this.removeRoles, this);
      this.addRoles = bind(this.addRoles, this);
      this.removeIdentityFromUser = bind(this.removeIdentityFromUser, this);
      this.addIdentityToUser = bind(this.addIdentityToUser, this);
      this._profileImageFromProfile = bind(this._profileImageFromProfile, this);
      this._displayNameFromProfile = bind(this._displayNameFromProfile, this);
      this._usernameFromProfile = bind(this._usernameFromProfile, this);
      this.getOrCreateUserFromProvider = bind(this.getOrCreateUserFromProvider, this);
      this.create = bind(this.create, this);
      this._hashPassword = bind(this._hashPassword, this);
      this.validateUserByUsernameOrEmail = bind(this.validateUserByUsernameOrEmail, this);
      this.findUserByUsernameOrEmail = bind(this.findUserByUsernameOrEmail, this);
      this.setPassword = bind(this.setPassword, this);
      this.destroy = bind(this.destroy, this);
      this["delete"] = bind(this["delete"], this);
      this.patch = bind(this.patch, this);
      this.getByNameOrId = bind(this.getByNameOrId, this);
      this.getByPrimaryEmail = bind(this.getByPrimaryEmail, this);
      this.getByName = bind(this.getByName, this);
      this.lookup = bind(this.lookup, this);
      this.getByUsernames = bind(this.getByUsernames, this);
      this.getByIds = bind(this.getByIds, this);
    ...
    UserMethods.prototype.getByPrimaryEmail = function(_tenantId, email, options, cb) {
      ...
      if (!email) {
        return cb(Boom.badRequest(i18n.errorEmailRequired));
      }
      if (_.isFunction(options)) {
        cb = options;
        options = {};
    ...
    UserMethods.prototype.patch = function(_tenantId, usernameOrId, obj, options, cb) {
      ...
      if (!usernameOrId) {
        return cb(Boom.badRequest(i18n.errorUsernameOrIdRequired));
      }
      if (_.isFunction(options)) {
        cb = options;
        options = {};
    ...
    UserMethods.prototype.setPassword = function(_tenantId, usernameOrId, password, options, cb) {
      ...
      if (!password) {
        return cb(Boom.badRequest(i18n.errorPasswordRequired));
      }
      if (_.isFunction(options)) {
        cb = options;
        options = {};
    ...
    UserMethods.prototype.findUserByUsernameOrEmail = function(_tenantId, usernameOrEmail, options, cb) {
      ...
      if (!usernameOrEmail) {
        return cb(Boom.badRequest(i18n.errorUsernameOrEmailRequired));
      }
      if (_.isFunction(options)) {
        cb = options;
        options = {};
    ...
    UserMethods.prototype.getOrCreateUserFromProvider = function(_tenantId, provider, v1, v2, profile, options, cb) {
      ...
      if (!_.isArray(options.roles)) {
        options.roles = ['user-needs-setup'];
      }
      _tenantId = mongooseRestHelper.asObjectId(_tenantId);
      identityQuery = {
        _tenantId: _tenantId,
      ...
      return this.models.User.findOne(identityQuery, (function(_this) {
        return function(err, user) {
          var i, identity, isUserNameValid, len, pusername, ref;
          if (err) {
            return cb(err);
          }
          if (user) {
            ...
            return _this.models.User.findOne({
              ...
              var email, emails, imageUrl, images, j, k, len1, len2, newIdentity, ref1, ref2, ref3, ref4, ref5;
              if (err) {
                return cb(err);
              }
              ...
              for (k = 0, len2 = emails.length; k < len2; k++) {
                email = emails[k];
              ...
              user.location = (ref1 = profile._json) != null ? (ref2 = ref1.location) != null ? ref2.name : void 0 : void 0;
              user.needsInit = !profile.username || !user.primaryEmail || user.primaryEmail.toLowerCase().indexOf("facebook.com") > 0;
              user.gender = profile.gender;
              user.timezone = (ref3 = profile._json) != null ? ref3.timezone : void 0;
              user.locale = (ref4 = profile._json) != null ? ref4.locale : void 0;
              user.verified = (ref5 = profile._json) != null ? ref5.verified : void 0;
    ...
    UserMethods.prototype.removeIdentityFromUser = function(userId, identityId, options, cb) {
      ...
      if (!identityId) {
        return cb(Boom.badRequest(i18n.errorIdentityIdRequired));
      }
      if (_.isFunction(options)) {
        cb = options;
        options = {};
    ...
    UserMethods.prototype.addRoles = function(userId, roles, options, cb) {
      ...
      if (!(roles && _.isArray(roles) && roles.length > 0)) {
        return cb(Boom.badRequest(i18n.errorRolesRequired));
      }
      if (_.isFunction(options)) {
        cb = options;
        options = {};
    ...
    UserMethods.prototype.resetPasswordToken = function(_tenantId, token, password, options, cb) {
      ...
      if (!token) {
        return cb(Boom.badRequest(i18n.errorTokenRequired));
      }
      if (!password) {
        return cb(Boom.badRequest(i18n.errorPasswordRequired));
      }
        ...
        return function(err, hash) {
          ...
          return _this.models.User.findOne({
            ...
            if ((user.resetPasswordToken.token || '').toLowerCase() !== token.toLowerCase()) {
              return cb(Boom.badRequest(i18n.errorTokenInvalid));
            }
            if (!(user.resetPasswordToken.validTill && user.resetPasswordToken.validTill.isAfter(new Date()))) {
              return cb(Boom.badRequest(i18n.errorValidTillFailed));
            }
